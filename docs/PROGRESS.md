...
## 2026-05-16 — `(this commit)` — P3.3: agent write tools via propose-then-approve

**Shipped:**
- Migration `0015_agent_actions` — dedicated audit table with status lifecycle (pending → executed | rejected), FK to chat_logs, FK to result_node, payload jsonb, reason text, RLS workspace-scoped.
- `tools.py` — `ToolContext.proposals` field added; new `propose_summary_node(title, content, source_node_ids, reason)` tool. Dispatch RECORDS INTENT only — appends to ctx.proposals, returns `{queued: true, summary: ...}` to the agent. Validates source_ids with the UUID regex from P3.1 (same defensive pattern; rejects label-as-uuid mistakes early).
- `agent.py` — `AgentDone.proposals` field, snapshotted from `ctx.proposals` at loop end, then cleared so reflection retries start fresh. System prompt updated to describe the write tool's proposal semantics ("does NOT write; the user reviews and approves").
- `routers/agent.py` — collects proposals across attempts, inserts `agent_actions` rows with `status='pending'` after the chat_logs row lands (so chat_log_id FK works), emits a new SSE `proposals` event with the persisted action ids before `done`.
- New endpoints `POST /agent/proposals/{id}/approve` + `/reject`. Approve: insert summary node with `metadata.created_by='agent'` + manual edges back to source nodes + transition `agent_actions.status='executed'`. Reject: just transition status. Both 404 if row isn't `status='pending'` (idempotency at the state-machine level).
- `ChatPanel.tsx` — new `AgentProposal` type with local `uiStatus` for optimistic transitions; new `ProposalsCard` + `ProposalRow` sub-components. Card renders below the answer when proposals exist; each row has Approve/Reject buttons, collapsible payload preview (title + content snippet + reason + source count). After approve: refetch nodes + edges from the store so the new node appears in the 3D canvas immediately.
- ADR-022 documenting the propose-then-approve design — why mid-loop modal pause was rejected (server-side loop can't wait on browser), why no undo button in v1 (sidebar delete is the existing affordance), what's deferred (propose_edge, propose_node_update, batch-approve UX, visual differentiation for agent-created nodes).

**What I learned:**
- **The "agent can write" boundary is a user-trust boundary first, a code boundary second.** The hard work in P3.3 wasn't the SQL or the tool dispatch — it was the protocol design. Mid-loop modal pause would have needed websockets, a half-complete agent state, and timeouts. Propose-then-approve keeps the agent loop pure-server-side, batches decisions for the user, and survives reflection retries because proposals snapshot per attempt and clear.
- **Idempotency lives at the state machine, not the request.** The approve endpoint doesn't dedupe by request id (no extra table). It just enforces `status='pending'` on the SELECT — re-approving an already-executed row is a 404. Same for reject. Simple, durable, no fancy distributed-systems machinery. Works because state transitions are one-way.
- **Audit tables are cheap insurance.** `agent_actions` is one row per proposal forever (pending/executed/rejected). Even rejected proposals stay — they're the user's "I considered this and said no" record. The next feature ("show me what the agent has been proposing this week") falls out for free. The cost is one INSERT per proposal; we already INSERT a chat_logs row per turn, so the overhead is in noise.
- **`metadata.created_by='agent'` is a quiet but useful breadcrumb.** Nodes don't visually differentiate in the canvas yet, but the metadata is durable. Future features (filter "show only my notes vs agent's", different glow, badge in sidebar) all sit on this single field. Add the breadcrumb when you have the chance; don't wait until you need it.
- **The propose-then-approve pattern is the substrate for P3.4 and P3.5.** Memory agent doesn't need new write tools — it uses `propose_summary_node` from this commit, just on its own initiative instead of in response to a user question. Research agent adds external-source tools but uses the same approval pipeline. Building the gate well in P3.3 was the unlock for everything autonomous that comes after.
- **Audit-driven amendment: approve handler now mirrors the note-save chain.** First real approval surfaced three issues: manual edges to source ids (wrong edge kind), 0 chunks on the new node (invisible to /chat), and the agent's source_ids treated as hard graph links. Fix: approve now does insert → embed → rebuild_semantic_edges (same chain the UI uses for note saves). The agent's source_node_ids become a `metadata.source_node_ids_hint` audit trail; semantic edges form via the kNN pipeline on real similarity. Two-line conclusion: agent-created notes are now indistinguishable from user-created notes in the graph except for the `metadata.created_by='agent'` flag. That's the right invariant. ADR-022 amended in-place.
- **Follow-up: auto-recompute clusters in the save chain.** Same-day request after the approve fix landed — the cluster legend went stale every time a new node was added (whether by user via Sidebar's DraftForm, file upload re-ingest, or agent approval). Extended the chain: `embed → rebuild_semantic_edges → recompute_clusters → refresh nodes/edges/clusters`. members_hash reuse keeps the cost bounded (~$0.0001 for the 1-2 changed clusters per add). New `Sidebar.autoConnectChain(wsId)` helper does the whole post-save sequence in one place. Agent approve handler does the cluster-recompute call client-side (in `ProposalsCard.approve` after the approve POST returns) so the approve button transitions fast and the legend updates a beat later — server-side approve stays snappy.
- **Neon glow on newest node.** The most-recently-created node (by `created_at` desc) renders as a single `THREE.Group` containing my own sphere mesh (amber `#fde047` MeshLambertMaterial with `emissiveIntensity: 0.45`) + an additive-blended halo sprite (~50 world units, soft falloff via a 256px radial-gradient canvas texture). Combined effect: visible bright sphere with a soft glow bleed on the dark background — reads as "highlighted/recent" without screaming "sun." **First two implementations failed and taught the right shape**: (i) initial "just bigger + brighter color" was too subtle; (ii) the next attempt used `nodeThreeObjectExtend={true}` with a separate sprite Group on top of the default sphere — looked like a literal sun because the lib's per-tick `nodePositionUpdate` only moves the default mesh; the sprite stayed at the d3-force initial position while the sphere drifted away in oscillation, so the same node rendered TWICE at different positions. Fix: REPLACE the default mesh entirely (extend=false) with one unified Group; per-tick nodePositionUpdate moves the Group, sphere + halo stay glued together at the right spot. Stays "newest" until a newer node arrives — no time-fade. Pulse animation skipped (sim cools, callback stops firing).
- **Follow-up rename: `propose_summary_node` → `create_note` (migration 0016).** The v1 tool name leaked a use-case assumption (summary) into the public surface. The tool just creates a note — users save lists, journal entries, plain notes, anything. Renamed schema name + dispatch function + `action_type` enum across `agent_actions`. Migration 0016 drops the CHECK constraint, UPDATEs existing rows (`'create_summary_node'` → `'create_note'`), re-adds the constraint with the new value. One canonical name everywhere; old audit rows rewrite cleanly. Substrate (propose-then-approve pipeline, approve handler, UI card) all stay exactly as they are — only the *name* changed.

---

## 2026-05-16 — `(this commit)` — P3.4: memory agent (button-triggered curation)

**Shipped:**
- **🧠 Curate memory button** in the canvas controls. Click fires a `window.dispatchEvent('mempalace:ask', {question, agent: true})` custom event with a hardcoded curation prompt.
- **ChatPanel custom-event listener** picks it up, opens the panel, runs the prompt through `/agent` exactly as if the user had typed it. Uses a `sendRef` pattern so the listener bound at mount always invokes the latest send closure (avoids stale-state bugs).
- **`send()` now accepts an optional override** — `{question, agent}` — letting external callers fire questions through whichever endpoint they want without persisting the agent-mode toggle state.
- **Curation prompt** is a 7-step instruction: list_clusters → for clusters with 3+ members, read members → skip already-summarized → propose up to 3 create_notes → end with a narrative.
- **ADR-023** explaining the no-new-endpoint design — "memory agent = agent with a specific opening question."
- ROADMAP P3 → ~80%. P3.5 (research agent w/ web fetch) is the only remaining piece.

**What I learned:**
- **Good substrate makes follow-on features small.** P3.1 (agent loop) + P3.2 (reflection) + P3.3 (write tools + propose-then-approve + audit) compounded so completely that P3.4 ships as ~40 lines of frontend code and zero backend. The pattern: each layer earns its place by NOT being narrowly scoped — read tools work for any agent task, the propose pipeline works for any write tool, reflection works for any answer. Build for the substrate, not the feature.
- **Sometimes the right design is "less than you think."** I caught myself sketching a `MemoryAgent` class, a dedicated `/memory-agent` endpoint, a `memory_agent_runs` table. None of it needed. The memory agent is literally a prompt. Calling it that prevents over-engineering.
- **Pub-sub via window events is a real React pattern.** I almost reached for `forwardRef` + `useImperativeHandle` for the button → ChatPanel call. CustomEvent is simpler, more decoupled, and lets future features (hotkeys, command palette, scheduled curate) tap the same channel without modifying ChatPanel's signature.
- **The sendRef pattern is the standard fix for "useEffect bound at mount needs latest state."** Capture the function in a ref, update the ref each render, dereference inside the effect callback. Avoids both stale closures and effect re-binding churn.
- **Audit-driven prompt tightening (same commit).** First live Curate run produced 4 proposals — two of them duplicate `Summary of Sumoo Gym Interests`. Tightened the prompt: explicit *"ONE create_note per cluster, period"* + *"if you find yourself proposing duplicates, drop the duplicate before responding"* + stronger skip rule for clusters that already contain a 'Summary'-titled member. Reflection retry tied at 2/5 on the bulk task — also an honest signal that "summarize everything" prompts don't benefit from retry, so could lower the retry threshold for the curate path later (deferred).

---

## 2026-05-16 — `(this commit)` — P3.5 v1: research agent (web_fetch only, no search)

**Shipped:**
- `services/web_fetch.py` (new) — `fetch_url(url) → FetchResult` with the full safety story baked in: SSRF prevention (resolves DNS, refuses private/loopback/link-local; re-checks after redirects), scheme restriction (http/https only), 10s timeout via httpx, 5MB size cap enforced during streaming (breaks early), content-type filter (text/html, text/plain, application/xhtml+xml), honest User-Agent. BeautifulSoup + lxml for main-content extraction; strips script/style/nav/header/footer/aside/form/button/svg/template; prefers `<article>` → `<main>` → `[role=main]` → `<body>` containers. Caps extracted text at 6000 chars so the agent's context doesn't balloon across multiple fetches.
- `tools.py` — `web_fetch(url)` tool schema + dispatch. Slim result shape: `{url, final_url, title, text, content_type, byte_length}` or `{error, url}` on failure (in-band errors, never raises).
- `agent.py` — system prompt updated. New "ONE EXTERNAL READ tool" section explaining when to use web_fetch ("when the user gives you a URL or names a specific source — don't browse on every question"). create_note section updated to call out "saving from a web fetch, include the source URL on the first line of the content so the user can trace it back."
- `requirements.txt` — added `beautifulsoup4==4.12.3` + `lxml==5.3.0` (~5MB combined to the API image).
- ADR-024 documenting the ship-web_fetch-defer-web_search choice + the full safety guard rationale.
- ROADMAP P3 → ~95% shipped. P3.5 v2 (search, PDFs, per-domain rate limits) carved out as deferred work behind the same architectural pattern.

**What I learned:**
- **The safety story for any "talks to the open internet" feature dwarfs the feature itself.** The actual `fetch + extract` logic is ~30 lines. SSRF prevention, scheme restriction, size cap, redirect re-validation, content-type filter, DNS-fail-closed — that's another 80 lines and most of the cognitive work. If you skip them you're shipping a vulnerability, not a feature.
- **Re-validate after redirects.** `http://attacker.com` could 302 to `http://169.254.169.254` (AWS metadata) — naive SSRF checks only validate the request URL, not where it lands. httpx exposes the final URL after redirects; re-running `_validate_url` on it closes the gap.
- **Defer the paid integrations.** Search APIs (Brave, Tavily, SerpAPI) all cost $5-50/mo. For a portfolio piece + first-real-user phase, shipping web_fetch alone validates the full external-source → graph loop without billing surprises. The architecture for adding search later is exactly one tool schema + one dispatch function — same shape as every other tool.
- **The propose-then-approve substrate kept this trivial.** Agent reads a URL, decides to save → uses the existing `create_note` tool → user sees the standard approval card. Web-sourced and memory-derived notes are indistinguishable in the downstream graph (only the agent's choice to include the source URL in the content text marks them).
- **Tool result caps matter doubly for external content.** A single web page can easily be 50k chars. The MAX_TEXT_CHARS=6000 cap is what keeps a 3-fetch research turn from blowing the context window. Same lesson as the chunk-preview caps in P3.1 — every result going into the message history needs an upper bound, especially when the source is unbounded (user-pasted URL).
- **Audit-driven follow-up: raised semantic-edge min_weight floor from 0.25 → 0.30.** Live observation after a research-agent fetch: the newly-saved web-sourced note picked up a low-weight semantic edge to an unrelated "Summary of Sumit" node — they shared a few keywords but weren't really about the same thing. The 0.25 floor was inherited from P2.5 when the corpus was tiny and we wanted *any* connections. With richer content (web fetches, agent-summaries, longer notes), 0.25-0.30 weight is mostly noise: real shared topics in this corpus reliably score 0.30+. Changed `min_weight` default in `routers/workspaces.py:rebuild_semantic_edges` + the agent approve handler + `lib/edgeTiers.ts:weak.min`. Doc strings in ROADMAP / ARCHITECTURE / LEARNING updated to reflect the new floor and note the audit-driven change. Next rebuild (auto-triggered on any new node) drops the existing 0.25-0.30 edges; no manual cleanup needed.
